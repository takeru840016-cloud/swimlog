...
JO基準データを格納・取得するDB構造とAPIはありますが、現在の画面はデータの登録有無を表示するだけです。JO標準の突破判定、基準タイムとの差、判定結果の表示は未実装です。
...
本リポジトリには、資格級・JO基準データは含まれていません。資格級判定を利用するには、利用者自身で正規に入手した資料をもとにCSVを作成し、D1へ投入してください。先に「D1 Migrationを適用」まで完了させておく必要があります。

JO基準データも同じテーブルへ保存できますが、現在の画面にはJO判定結果が表示されません。以下の投入手順は、現時点では資格級判定を利用するための手順です。
...
資格級CSVではD1の `system` を `grade` として投入します。DB構造上はJO基準も保存でき、その場合は同じCSV形式で後述の `$System = "JO"` を指定します。ただし、JO基準を投入しても現在の画面でJO判定結果は表示されません。資格級とJOは別々のCSV・SQLにしてください。
...
$System = "grade" # JOはDB保存のみ可能。現在、判定結果の画面表示は未実装
